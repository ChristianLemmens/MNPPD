...
try:
    csv_file = str('Output/long/' + sys.argv[1] +'/'+ sys.argv[2] +'/priors.csv')
...
# DEFINE VARIABLES: [name,value, min, max, #comp, #variab] ## REMOVE VALUES


listvar = [C2_values,Tmidmin_values,
...
def set_parameters(transformations, values):
    # transform the cube value in readable (physical) value and set to your model
    """
    |PyMN|MNPPD| Set current parameters.
    """
    # transfomations is a dict with [model, index, name, transform, aftertransform]
    pars = rimparavalues['No.'][0:3].to_list() #add inner rim from short wavelength analysis
    ...
    # model = model_complete(pars,wavrange)
    model = f.components['disk_model'] + s.stellar

    return model

# (For Chris)
# daniele didn't change it. It analyses the resulting chain
...
# Collect Inner Rim parameters from shortwavelength regime
rimparavalues = pd.read_csv('Output/short/' + sys.argv[1] + '/' + sys.argv[2] + '/test-stats.dat',sep='\s+',skiprows=2,nrows=3,index_col=False,usecols=[1])
...
output_folder = 'Output/long/' + sys.argv[1] + '/'
try:
    os.mkdir(output_folder)
    print("New Directory created")
except:
    print("Using folder: long/" + sys.argv[1])
